...
<script>
    ...
    function listWorkPlan(data) {
        ...
        data.workPlanMachine.products.forEach(e => {
            ...
            html += '</div>'
            html += '<div class="col align-self-center text-center">'
            var dataConvert = convertToTarget(e)
            html += '<p class="m-0 super-small-text">' + dataConvert.qty + ' ' + dataConvert.unit_name + '</p>'
            html += '</div>'
            html += '</div>'
            html += '</div>'
            html += '</div>'
    ...
    function convertToTarget(data) {
        var unit_asal = data.unit_id
        var dataUnit = data.unit_option.find((v, k) => {
            if (v.id == unit_asal) return true
        })
        eval('var convertToDefault = data.qty ' + dataUnit.operator_reverse + ' ' + dataUnit.multiplier)
        var unit_target = data.unit_target.id
        var dataUnitTarget = data.unit_option.find((v, k) => {
            if (v.id == unit_target) return true
        })
        eval('var convertToTarget = convertToDefault ' + dataUnitTarget.operator + ' ' + dataUnitTarget.multiplier)
        return {
            qty: roundToTwo(convertToTarget),
            unit_id: unit_target,
            unit_name: dataUnitTarget.name,
            unit_multiplier: dataUnitTarget.multiplier
        }
    }
