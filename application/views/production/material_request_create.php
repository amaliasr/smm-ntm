            ...
            <div class="card-body p-3">
                <div class="row p-0">
                    <div class="col align-self-center text-end">
                        <p class="m-0"><b>Total Material : </b><span class="text-success" id="totalMaterial">0</span></p>
                    </div>
...
<script>
    ...
    function getData(data, url) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // find Actived Machine
                data_plan.productionPlan.forEach(a => {
                    a.detail.forEach(b => {
                        b.data.forEach(c => {
                            activedMachine.push(c.machine.id)
                        });
                    });
                });
                // find Actived Machine
    ...
    function dataMaterialRequest() {
        ...
        $.each(data, function(key, value) {
            ...
            $.each(value['detail']['machine'], function(keys, values) {
                ...
                var iconActive = ''
                if (checkMachinePlan) {
                    iconActive = '<i class="fa fa-check-circle text-success ms-1" title="Planned Machine"></i>'
                }
                html += '<th class="p-2 font-small text-nowrap">' + values['code'] + ' ' + iconActive + '</th>'
            })
            html += '</tr>'
            html += '</thead>'
            html += '<tbody>'
    ...
    function collectDataInput() {
        ...
        $('#totalMaterial').html(filled_cell.length)
        checkFilledForm()
    }
